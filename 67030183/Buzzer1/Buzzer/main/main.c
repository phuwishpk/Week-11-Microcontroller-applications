...
void app_main(void)
{
    ...
    ESP_LOGI(TAG, "Start LDR + Buzzer test");
    ...
    while (1) {
        ...
        // ตรวจเงื่อนไข (ค่าต่ำกว่า 1000 -> มืด -> Buzzer ดัง)
        if (raw < 1000) {
            gpio_set_level(BUZZER_PIN, 1);  // เปิด Buzzer
            ESP_LOGI(TAG, "Buzzer ON (Dark detected!)");
        } else {
            gpio_set_level(BUZZER_PIN, 0);  // ปิด Buzzer
            ESP_LOGI(TAG, "Buzzer OFF (Bright enough)");
        }
